...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // if (distanceToLight2 < distanceToLight) {
    //     distanceToLight = distanceToLight2;
    // }
    ...
    // --
    // vec2 lightPosition2;
    // lightPosition2.x = 320;
    // lightPosition2.y = 320;
    // normalizedFragCoord = fragCoord/iResolution.xy;
    // normalizedLightCoord = lightPosition2.xy/iResolution.xy;
    // float lightAmount2 = 1.0;
    // for(float i = 0.0; i < N; i++)
    // {
    //     // A 0.0 - 1.0 ratio between where our current pixel is, and where the light is
    ...
    // lightAmount2 *= 1.0 - smoothstep(0.0, lightSize, distanceToLight);

    // if (lightAmount2 < lightAmount) {
    //     lightAmount = lightAmount2;
    // }
    ...
    fragColor = mix(blackColor, texture(iChannel1, normalizedFragCoord), lightAmount + lightAmount2);
}
